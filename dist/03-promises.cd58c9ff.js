...
function onPromiseCreate(e) {
    e.preventDefault();
    const { delay: delay1 , step , amount  } = e.currentTarget.elements;
    let inputDelay = Number(delay1.value);
    let inputStep = Number(step.value);
    let inputAmount = Number(amount.value);
    for(let i = 1; i <= inputAmount; i += 1){
        inputDelay += inputStep;
        createPromise(i, inputDelay).then(({ position , delay  })=>{
            (0, _notiflixNotifyAio.Notify).success(`✅ Fulfilled promise ${position} in ${delay}ms`, options);
        }).catch(({ position , delay  })=>{
            (0, _notiflixNotifyAio.Notify).failure(`❌ Rejected promise ${position} in ${delay}ms`, options);
        });
